...
1. **現状確認**: `list_tables` で対象テーブルの現状を確認。RLS関連なら `SECURITY.md` と `.claude/rules/db-and-rls.md` を先に読む。
...
- DB履歴とローカルファイルは乖離している（`supabase/migrations/CLAUDE.md` 参照）。`supabase db push` / `db reset` は使わない。
