...
export function NewsRoomsPageClient({
  ...
  const isLocalUser = Boolean(localAd);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(
      (post) =>
        ...
        (selectedCategory === "update" && post.category === "announce"),
    );
  }, [featuredTop, posts, selectedCategory]);

  const visiblePosts = filteredPosts.slice(0, visibleCount);
  const feedWithAds = useMemo(
    () => injectAdsIntoFeed(visiblePosts, isLocalUser),
    [isLocalUser, visiblePosts],
  );
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[190px_minmax(0,1fr)_300px]">
            ...
            {CATEGORY_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[11px] ${active ? "text-white/70" : "text-slate-400"}`}
                  >
                    {item.key === "featured"
                      ? featuredTop.length
                      : item.key === "all"
      ...
      <section className="space-y-4">
        {nationalAd ? <AdCard ad={nationalAd} size="medium" /> : null}
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
          <div className="border-b border-slate-200 px-6 py-5">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-600">
              一覧で理解して、気になる記事を深く読み、次の行動へつなげるニュース体験です。
            </p>
          </div>
          <div className="grid gap-4 p-4 lg:grid-cols-3">
            {featuredTop.map((post, index) => (
              ...
              >
                {primaryImage(post) ? (
                  <Image
                    src={primaryImage(post) ?? ""}
                    alt={post.title}
                    width={index === 0 ? 800 : 400}
                    height={index === 0 ? 260 : 160}
                    unoptimized
                    className={`w-full object-cover ${index === 0 ? "h-[260px]" : "h-[160px]"}`}
                  />
                ) : (
                  <div
                    className={`bg-[linear-gradient(135deg,#dbeafe,#eff6ff)] ${index === 0 ? "h-[260px]" : "h-[160px]"}`}
                  />
                )}
                <div className="p-5">
                  <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px]">
                    ...
                    <span
                      className={`rounded-full border px-3 py-1 font-bold ${roleColor(post.authorRole)}`}
                    >
                      {post.authorRole}
                    </span>
                    <span className="text-slate-500">
                      {NEWS_TOPIC_LABEL[post.topic]}
                    </span>
                  </div>
                  <h2
                    className={`font-black leading-tight text-slate-950 ${index === 0 ? "text-[26px]" : "text-lg"}`}
                  >
                    {post.title}
                  </h2>
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
            ...
            <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-2xl font-black text-slate-950">
                  {CATEGORY_ITEMS.find((item) => item.key === selectedCategory)
                    ?.label ?? "すべて"}
                </h2>
                <p className="mt-1 text-sm text-slate-600">
                  {filteredPosts.length.toLocaleString()}本の記事
                </p>
              </div>
          ...
          <div className="divide-y divide-slate-200">
            {feedWithAds.map((item) => {
              if (isAdSlot(item)) {
                ...
                const ad = ads.find((a) => a.plan === tier) ?? null;
                if (!ad) return null;
                ...
                >
                  {primaryImage(post) ? (
                    <Image
                      ...
                      height={170}
                      unoptimized
                      className="h-[170px] w-full object-cover sm:h-full"
                    />
                  ) : (
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <h3 className="text-[24px] font-black leading-tight text-slate-950">
                      {post.title}
                    </h3>
                    <p className="mt-3 line-clamp-2 text-sm leading-7 text-slate-600">
                      {excerpt(post.summary || post.body, 120)}
                    </p>
          ...
          <div
            ref={loaderRef}
            className="px-6 py-5 text-center text-sm text-slate-500"
          >
            ...
            {trendingPosts.map((post, index) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-slate-500">
                  閲覧 {post.viewCount.toLocaleString()}
                </p>
              </Link>
            ))}
          </div>
