...
import { updateTrainerSession } from "@/lib/supabase/trainer-hub";
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const summary = await updateTrainerSession(profile, id, parsed.data);
    ...
    return NextResponse.json({ success: false, error: message || "セッション更新に失敗しました" }, { status: 500 });
  }
}
