...
// Journey の画像 / 動画アップロード。profiles バケットの journeys/ 配下へ保存し公開URLを返す。
export async function POST(req: NextRequest) {
    ...
    if (!allowed.has(file.type)) {
        return NextResponse.json(
            { error: isImage ? "JPEG / PNG / WebP / GIF / AVIF をアップロードしてください" : "MP4 / WebM / MOV をアップロードしてください" },
            { status: 400 },
        );
    }
    ...
    const path = `journeys/${user.slug}/${kind}-${Date.now()}.${ext}`;
    ...
    if (upload.error) {
        console.error("[journey/upload]", upload.error);
