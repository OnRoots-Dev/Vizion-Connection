...
export function VizMapView({ t, onBack }: { t: ThemeColors; roleColor: string; onBack: () => void }) {
    ...
    return createPortal(<section className="fixed inset-0 z-50 bg-[#09090f]" aria-label="Viz Map">
        ...
        <GestureSheet open={Boolean(selected || selectedPerson || selectedMonetize || selectedCluster.length)} onClose={() => { setSelectedId(null); setSelectedPerson(null); setSelectedMonetize(null); setSelectedCluster([]); }} side={desktopDetails ? "right" : "bottom"} snapHeights={desktopDetails ? undefined : ["25dvh", "52dvh"]} snap={sheetSnap} onSnapChange={setSheetSnap} className="px-5 pb-[max(20px,env(safe-area-inset-bottom))]">{selected ? <MapContextHeader label={selected.entity_type === "moment" ? "Around this moment" : "Around this activity"} count={1} dominant={PIN_COLOR_LABEL[categoryOf(selected)]} /> : selectedPerson ? <MapContextHeader label="Around this person" count={1} dominant={PIN_COLOR_LABEL[ROLE_CATEGORY[selectedPerson.author_role ?? ""] ?? "athlete"]} /> : selectedMonetize ? <MapContextHeader label="Sponsored nearby" count={1} dominant="Business" /> : selectedCluster.length ? <MapContextHeader label="Around here" count={selectedCluster.length} dominant={(() => { const grouped = new Map<string, number>(); for (const item of selectedCluster) { const key = item.type || "activity"; grouped.set(key, (grouped.get(key) ?? 0) + 1); } return [...grouped.entries()].sort(([, a], [, b]) => b - a)[0]?.[0] ?? "Activity"; })()} /> : null}{selected ? <ContentSheet item={selected} expanded={desktopDetails || sheetSnap === "half"} onExpand={() => setSheetSnap("half")} onComments={() => setCommentMomentId(selected.entity_type === "moment" ? selected.id : null)} /> : selectedPerson ? <PersonSheet item={selectedPerson} /> : selectedMonetize ? <MonetizeSheet pin={selectedMonetize} /> : selectedCluster.length ? <ClusterSheet items={selectedCluster} onOpen={(item) => { setSelectedCluster([]); setSelectedId(item.id); setSheetSnap("peek"); toast.show({ title: "Activity detail opened", description: item.title || "Nearby activity detail", tone: "success" }); }} /> : null}</GestureSheet>
