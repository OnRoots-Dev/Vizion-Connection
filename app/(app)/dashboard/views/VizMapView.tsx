...
export function VizMapView({ t, initialPrefecture, onBack }: { t: ThemeColors; roleColor: string; initialPrefecture?: string | null; onBack: () => void }) {
    ...
    return createPortal(<section className="fixed inset-0 z-50 bg-[#09090f]" aria-label="Viz Map">
        ...
        <header className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-2 p-4">
            <button type="button" onClick={onBack} aria-label="戻る" title="戻る" className="pointer-events-auto flex h-10 w-10 items-center justify-center rounded-[8px] border bg-transparent text-white/80 backdrop-blur transition active:scale-[0.98]" style={{ border: "1px solid rgba(255,255,255,0.16)", background: "rgba(0,0,0,0.28)" }}><svg width={16} height={16} fill="none" viewBox="0 0 24 24" stroke="rgba(255,255,255,0.75)" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg></button>
