...
export function VizMapView({ t, onBack }: { t: ThemeColors; roleColor: string; onBack: () => void }) {
    ...
    return createPortal(<section className="fixed inset-0 z-50 bg-[#09090f]" aria-label="Viz Map">
        ...
        <Link href="/dashboard?view=activities" className="absolute bottom-6 left-4 z-10 inline-flex min-h-11 items-center rounded-xl border border-white/15 bg-black/75 px-4 text-xs font-bold text-white backdrop-blur">＋ Create</Link>
        <GestureSheet open={Boolean(selected || selectedPerson)} onClose={() => { setSelectedId(null); setSelectedPerson(null); }} side={desktopDetails ? "right" : "bottom"} snapHeights={desktopDetails ? undefined : ["25dvh", "52dvh"]} snap={sheetSnap} onSnapChange={setSheetSnap} className="px-5 pb-[max(20px,env(safe-area-inset-bottom))]">{selected ? <ContentSheet item={selected} expanded={desktopDetails || sheetSnap === "half"} onExpand={() => setSheetSnap("half")} onComments={() => setCommentMomentId(selected.entity_type === "moment" ? selected.id : null)} /> : selectedPerson ? <PersonSheet item={selectedPerson} /> : null}</GestureSheet>
