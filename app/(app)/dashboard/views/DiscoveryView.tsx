...
export function DiscoveryView({ profile, t, roleColor, setView, ads }: {
    profile: ProfileData;
    t: ThemeColors;
    roleColor: string;
                ...
                <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", border: `1px solid ${roleColor}25`, background: "linear-gradient(145deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01))", padding: 14 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                        <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                            ...
                            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="名前 or @slug" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1px solid ${t.border}`, background: "rgba(255,255,255,0.03)", color: t.text, fontSize: 12 }} />
                        </label>
                        <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        ...
                        <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                            <span style={{ fontSize: 9, color: t.sub, opacity: 0.8 }}>Region</span>
                            <input value={region} onChange={(e) => setRegion(e.target.value)} placeholder="region" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1px solid ${t.border}`, background: "rgba(255,255,255,0.03)", color: t.text, fontSize: 12 }} />
                        </label>
                        <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                            <span style={{ fontSize: 9, color: t.sub, opacity: 0.8 }}>Prefecture</span>
                            <input value={prefecture} onChange={(e) => setPrefecture(e.target.value)} placeholder="prefecture" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1px solid ${t.border}`, background: "rgba(255,255,255,0.03)", color: t.text, fontSize: 12 }} />
                        </label>
                    </div>
                ...
                <SectionCard t={t}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                        {picks.cheer.slice(0, 5).map((u, i) => (
                            <motion.a key={`c-${u.slug}`} href={`/u/${u.slug}`} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} style={{ textDecoration: "none" }}>
                ...
                <SectionCard t={t}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                        {picks.referral.slice(0, 5).map((u, i) => (
                            <motion.a key={`r-${u.slug}`} href={`/u/${u.slug}`} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} style={{ textDecoration: "none" }}>
                ...
                {loading ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 10 }}>
                        {users.flatMap((u, i) => {
                            ...
                            nodes.push(
                                <motion.a key={`user-${u.slug}-${i}`} href={`/u/${u.slug}`} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} style={{ textDecoration: "none" }}>
                                    <div style={{ position: "relative", overflow: "hidden", borderRadius: 14, padding: "12px", background: "linear-gradient(155deg, rgba(255,255,255,0.04), rgba(255,255,255,0.015))", border: `1px solid ${rc}40` }}>
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 8 }}>
                                            <div style={{ width: 36, height: 36, borderRadius: "50%", overflow: "hidden", background: `${rc}25`, border: `1px solid ${rc}40`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, color: rc, flexShrink: 0 }}>
                                                {u.avatar_url ? <img src={u.avatar_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : u.display_name?.[0]?.toUpperCase()}
                                            </div>
                                            <div style={{ minWidth: 0, flex: 1 }}>
                                                <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.display_name}</p>
                                                <p style={{ margin: 0, fontSize: 9, color: t.sub, opacity: 0.7 }}>@{u.slug}</p>
                                            </div>
                                        </div>
                                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                                            ...
                                            <span style={{ fontSize: 8, fontFamily: "monospace", padding: "2px 6px", borderRadius: 999, background: "rgba(255,255,255,0.06)", color: t.sub }}>{u.region ?? "-"}{u.prefecture ? `/${u.prefecture}` : ""}</span>
                                        </div>
