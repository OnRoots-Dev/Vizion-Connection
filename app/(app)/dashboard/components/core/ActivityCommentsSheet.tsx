...
// dashboard/components/core/ActivityCommentsSheet.tsx
// Activityコメント: 一覧 / 入力 / 投稿 / 削除（自分の分）。
// CommentsSheet（Moment用）と同型。対象が activity_comments である点のみ異なる。

import { useCallback, useEffect, useState } from "react";
import { AnimatePresence, motion, useReducedMotion } from "framer-motion";
import { apiGet, apiSend, ApiError } from "@/lib/api/core-client";
import type { ActivityCommentRecord } from "@/features/activity/types";
import type { ThemeColors } from "../../types";

type CommentRow = ActivityCommentRecord & {
    author_slug: string | null;
    author_display_name: string | null;
};

export function ActivityCommentsSheet({
    open,
    activityId,
    viewerId,
    t,
    ...
    async function remove(commentId: string) {
        ...
        setError("");
        try {
            await apiSend(`/api/activities/${activityId}/comments/${commentId}`, "DELETE");
            await load();
        } catch (e) {
