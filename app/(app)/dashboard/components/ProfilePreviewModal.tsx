...
export function ProfilePreviewModal({
  ...
  return (
    <GestureSheet open={Boolean(slug)} onClose={onClose} side="bottom">
      <div className="flex min-h-0 flex-col" style={{ maxHeight: "min(88dvh, 880px)" }}>
        <div
          className="sticky top-0 z-[2] flex items-start justify-between gap-3 border-b border-white/10 px-4 py-3 sm:px-5"
          style={{
            background: "rgba(10,10,10,0.72)",
            backdropFilter: reduce ? "none" : "blur(20px) saturate(160%)",
            WebkitBackdropFilter: reduce ? "none" : "blur(20px) saturate(160%)",
          }}
        >
          <div className="min-w-0 flex-1">
            ...
            <h2 className="mt-1 break-words text-[18px] font-black leading-tight tracking-[-0.02em] text-white sm:text-[20px]">公開イメージ</h2>
          </div>
          <motion.button
            type="button"
            onClick={onClose}
            ...
            className="shrink-0 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-white/75"
          >
            閉じる
          </motion.button>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain">
            <div className="relative min-h-[280px] overflow-hidden px-6 pb-7 pt-6 sm:min-h-[300px]">
              {profile.profileImageUrl ? (
                ...
                <div className="absolute inset-0" style={{ background: `linear-gradient(145deg, ${roleColor}22 0%, #0A0A0A 80%)` }} />
              )}
              <div className="absolute inset-0" style={{ background: "linear-gradient(to top, #0A0A0A 0%, rgba(10,10,10,0.24) 55%, rgba(10,10,10,0.06) 100%)" }} />
              ...
              <div className="relative z-[2] flex flex-col gap-5">
                ...
                <div className="flex items-end gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="m-0 break-words text-[28px] font-black leading-[1.05] tracking-[-0.02em] text-white sm:text-[34px]">{profile.displayName}</p>
                    <p className="mt-2 break-words text-[12px] leading-relaxed text-white/55">{profile.region ?? "Region not set"}{profile.sport ? ` · ${profile.sport}` : ""}</p>
            ...
            <div className="relative z-[3] -mt-6 grid gap-4 rounded-t-[28px] border-t border-white/10 bg-[#0A0A0A] px-4 py-4 sm:-mt-8 sm:px-6 sm:py-5 md:grid-cols-[1.1fr_0.9fr]">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    {highlightStats.map((stat, index) => (
                      <div key={`${stat.label}-${index}`} className="rounded-[18px] border border-white/10 bg-white/[0.03] p-4">
                        ...
                        <p className="m-0 text-[22px] font-black tracking-[-0.02em]" style={{ color: stat.color === "gold" ? "#FFD600" : stat.color === "role" ? roleColor : "#ffffff" }}>{stat.value}</p>
                      </div>
                    ))}
                  </div>
                ) : null}
              ...
              <div className="space-y-4">
                <div className="rounded-[22px] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <motion.button type="button" onClick={() => void handleCheer()} whileTap={press} transition={springDefault} disabled={cheerLoading}
                      className="inline-flex items-center gap-2 rounded-[16px] border px-[16px] py-[12px] font-black text-white"
                      style={{ borderColor: "rgba(255,214,0,0.28)", background: "linear-gradient(135deg, rgba(255,214,0,0.28), rgba(255,255,255,0.08))" }}>
                      <span className="text-[#FFD600]">📣</span> Cheer
                      <span className="rounded-full bg-black/25 px-2 py-1 font-mono text-[11px] text-white/90">{profile.cheerCount ?? 0}</span>
                    </motion.button>
                    <motion.button type="button" onClick={() => setShowCheerComment((p) => !p)} whileTap={press} transition={springDefault}
                      className="inline-flex items-center gap-2 rounded-[16px] border border-white/10 bg-white/[0.06] px-[16px] py-[12px] font-black text-white/90">
                      💬 コメント付きCheer
                    </motion.button>
                    <motion.button type="button" onClick={handleCollectToggle} whileTap={press} transition={springDefault} disabled={collectLoading}
                      className="inline-flex items-center gap-2 rounded-[16px] border px-[16px] py-[12px] font-black text-white"
                      style={{ borderColor: `${roleColor}33`, background: collected ? `linear-gradient(135deg, ${roleColor}33, rgba(255,255,255,0.08))` : "rgba(255,255,255,0.06)" }}>
                      {collected ? "Collected" : "Collect"}
                      <span className="rounded-full bg-black/25 px-2 py-1 font-mono text-[11px]">{collectorCount}</span>
                    </motion.button>
                  </div>
                  {showCheerComment ? (
                    <div className="mt-3 grid gap-2">
                      <textarea value={cheerComment} onChange={(e) => setCheerComment(e.target.value.slice(0, 120))} rows={3}
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <motion.button type="button" whileTap={press} onClick={() => void handleCheer(cheerComment)}
                          disabled={cheerLoading || !cheerComment.trim()}
                          className="rounded-xl border border-white/10 bg-white/5 px-[14px] py-[10px] text-[12px] font-bold text-white/90 disabled:opacity-50">
                ...
                <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <Link href={`/u/${profile.slug}`} onClick={onClose}
                    className="inline-flex w-full items-center justify-center rounded-xl px-4 py-3 text-center text-[13px] font-black no-underline active:scale-[0.97] transition-transform"
